...
client = Elasticsearch("https://localhost:9200", api_key=API_KEY, ca_certs="./ca.crt")
...
def findInfo(page):
    ...
    for res in results_arr:
        ...
        journals = [
            journal.get_text(strip=True) for journal in journal_tag.findAll("span")
        ]  # journals
        ...
        if bottom_tag:
            for tag in bottom_tag:
                ...
                if type == "Journal ref:":
                    ...
                    comments = tag.find(
                        "span", class_="has-text-grey-dark mathjax"
                    ).get_text(strip=True)
...
def insert_documents(documents, index):
    ...
    for document in documents:
        ...
        operations.append({**document, "embedding": getEmbedding(document["summary"])})
        # print(operations[1])
        # break
